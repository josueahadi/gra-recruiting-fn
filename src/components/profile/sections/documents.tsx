...
interface DocumentsSectionProps {
	uploadedCV: Document | null;
	uploadedSamples: Document[];
	portfolioLinks: PortfolioLinks;
	userType: "applicant" | "admin";
	onCVUpload?: (file: File) => void;
	onSampleUpload?: (files: FileList) => void;
	onCVRemove?: () => void;
	onSampleRemove?: (index: number) => void;
	onLinksUpdate?: (links: PortfolioLinks) => void;
}

/**
 * Reusable documents and portfolio section component that works for both applicant and admin views
 */
const DocumentsSection: React.FC<DocumentsSectionProps> = ({
	...
	const [updatedLinks, setUpdatedLinks] =
		useState<PortfolioLinks>(portfolioLinks);
	...
	const handleCVUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (canEdit && files && files.length > 0 && onCVUpload) {
			setIsUploading(true);
	...
	const handleSamplesUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		const files = e.target.files;
		if (canEdit && files && files.length > 0 && onSampleUpload) {
			setIsUploading(true);

			// In a real app, this would be an API call
			onSampleUpload(files);
	...
	return (
		<>
			...
			<ProfileInfoSection
				title="Resume"
				canEdit={false} // No edit button needed for this section
			>
						...
						<div className="mb-4 space-y-2">
							{uploadedSamples.map((sample, index) => (
								...
								>
									...
									{canEdit && onSampleRemove && (
										<Button
											variant="ghost"
											onClick={() => onSampleRemove(index)}
											className="text-red-500 hover:text-red-700 p-1 h-auto"
										>
											<X className="h-4 w-4" />
										</Button>
