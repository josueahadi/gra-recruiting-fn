...
export function AuthCheck({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
			...
			if (isAuthenticated && token) {
				...
				if (
					...
					const dashboardPath = isAdminRole(role)
						...
						: "/applicant/dashboard";
					console.log(
						"[AuthCheck] Redirecting from auth page to:",
						dashboardPath,
					);
		...
		if (isProtectedRoute(pathname) && !isAuthenticated) {
			console.log(
				"[AuthCheck] Unauthorized access to protected route:",
				pathname,
			);
			setIsRedirecting(true);
		...
		if (pathname.startsWith("/admin") && token) {
			...
			if (!isAdminRole(role)) {
				console.log(
					"[AuthCheck] Non-admin user attempting to access admin route",
				);
				setIsRedirecting(true);
		...
		if (pathname.startsWith("/applicant") && token) {
			...
			if (isAdminRole(role)) {
				console.log(
					"[AuthCheck] Admin user attempting to access applicant-specific route",
				);
				setIsRedirecting(true);
